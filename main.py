...
def init_db():
    conn = sqlite3.connect('ekub_final_pro.db')
    c = conn.cursor()
...
async def get_acc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("ለፕሮፋይልዎ ባለ 4 አሃዝ ሚስጥር ቁጥር (Password) ይፍጠሩ፦")
    return PASS
...
async def get_pay_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    bank = "Telebirr 0954873397 (Drbew)" if update.callback_query.data == "p_tele" else "CBE 1000536009276 (Gizachew)"
...
async def get_screenshot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = sqlite3.connect('ekub_final_pro.db')
    c = conn.cursor()
    c.execute("INSERT OR REPLACE INTO users (user_id, info, password, status, weekly_pay) VALUES (?, ?, ?, ?, ?)", 
              (user_id, info, password, "Pending", "Not Paid"))
    conn.commit()
    conn.close()
...
async def show_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user and str(user[1]) == str(entered_pass):
        ...
        await update.message.reply_text("❌ የተሳሳተ ሚስጥር ቁጥር! እንደገና ይሞክሩ /start")
    return ConversationHandler.END
...
async def admin_verify_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "reg":
        ...
        await context.bot.send_message(user_id, "✅ ምዝገባዎ ተረጋግጧል! አሁን አባል ሆነዋል።")
        await query.edit_message_caption(caption="🟢 ምዝገባ ተረጋግጧል")
    elif action == "week":
        ...
        await query.edit_message_caption(caption="🟢 የሳምንት ክፍያ ተረጋግጧል")
...
async def ekub_pay_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    bank = "Telebirr 0954873397 (Drbew)" if update.callback_query.data == "pay_e_tele" else "CBE 1000536009276 (Gizachew)"
    await update.callback_query.message.reply_text(f"በ {bank} የእቁብ ክፍያውን ፈጽመው ደረሰኙን (Screenshot) ይላኩ፦")
...
async def get_ekub_screenshot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_photo(ADMIN_ID, photo=update.message.photo[-1].file_id, caption=f"💰 የሳምንት ክፍያ ደረሰኝ\nከ: {update.effective_user.first_name}", reply_markup=verify_kb)
    await update.message.reply_text("ክፍያው እስኪረጋገጥ ትንሽ ይጠብቁ።", reply_markup=main_menu_keyboard(user_id))
    return ConversationHandler.END
