...
async def get_father_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "✨ እባክዎ **ስልክ ቁጥር ያስገቡ፦**\n(ከታች ያለውን አረንጓዴ አዝራር መጫን ወይም መጻፍ ይችላሉ)",
...
async def car_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(
        ...
        f"🎟 እባክዎ ከ **1 እስከ 1000** ያለ የእጣ ቁጥር ይምረጡና ቁጥሩን ብቻ ይጻፉልኝ (ለምሳሌ: `7`)፦",
        parse_mode="Markdown"
    )
    return TICKET_NUM
...
async def main():
    ...
    conv_handler = ConversationHandler(
        entry_points=[CommandHandler("start", start), MessageHandler(filters.TEXT & (filters.Regex("^🎟 እጣ ይቁረጡ$") | filters.Regex("^🔄 ሌላ እጣ ይቁረጡ$")), handle_main_menu)],
        states={
