...
# ለእያንዳንዱ መኪና የተሸጡ እጣዎች በተናጠል ይቀመጣሉ (አንድ ቁጥር ሌላ መኪና ላይ መቆረጥ ይችላል)
sold_tickets = {"ሀይሩፍ": set(), "አይሱዙ": set()}

# Conversation States
NAME, FATHER_NAME, PHONE_NUM, CHOOSE_CAR, TICKET_NUM, CHOOSE_PAYMENT, RECEIPT = range(7)
...
async def get_father_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✨ እባክዎ **ስልክ ቁጥር ያስገቡ፦**", parse_mode="Markdown")
    return PHONE_NUM
...
async def car_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(
        ...
        f"🎟 እባክዎ ከ **1 እስከ 1000** ያለ የእጣ ቁጥር ይምረጡና ቁጥሩን ብቻ ይጻፉልኝ (ለምሳሌ: `77`)፦",
        parse_mode="Markdown"
    )
    return TICKET_NUM
...
async def check_ticket(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # አንድ ቁጥር በዚህ መኪና ላይ ከተያዘ ይከለክላል፤ ሌላ መኪና ላይ ግን ይፈቅዳል
    if ticket_num in sold_tickets[selected_car]:
        await update.message.reply_text(f"❌ ይቅርታ፣ የእጣ ቁጥር {ticket_num} ለ{selected_car} አስቀድሞ ተሽጧል! እባክዎ ሌላ ቁጥር ይምረጡ፦")
        return TICKET_NUM
    ...
    await update.message.reply_text(
        ...
        f"👇 እባክዎ ክፍያ መፈጸም የሚፈልጉበትን የክፍያ መንገድ ይምረጡ፦",
...
async def get_receipt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "🕵️ አድሚን ደረሰኝዎን አረጋግጦ ሲጨርስ የማረጋገጫ መልዕክት ይደርስዎታል። እናመሰግናለን! 🙏",
