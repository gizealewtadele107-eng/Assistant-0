...
# --- 1. ቦቱ ሳይዘጋ እንዲቆይ የሚያደርግ (Keep-Alive) ---
app = Flask('')
@app.route('/')
...
# --- 2. ዋና ቅንብሮች ---
TOKEN = "8687440246:AAFbRwhQNjkbdC7Gd5UtuY1Tp6aBhKcg9r4" # አዲሱ Token እዚህ ገብቷል
ADMINS = [8442116232, 8687440246, 7705713321] # የእርስዎ የአድሚን ID
CAR, F_NAME, L_NAME, PHONE, TICKET, PAYMENT, SCREENSHOT, QUESTION, BROADCAST = range(9)
CAR_DATA = {"ሀይሩፍ": "2500 BIRR", "አይሱዙ": "3000 BIRR"}
PAYMENT_INFO = {"Telebirr": "ቁጥር: 0974671533", "CBE": "አካውንት: 1000701313821"}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    init_db()
    kb = [["ሀይሩፍ", " አይሱዙ"], ["❓ ጥያቄ ለመጠየቅ"]]
    if update.effective_user.id in ADMINS: 
        kb.append(["📢 ማስታወቂያ ላክ"])
    
    await update.message.reply_text(
        "👋 እንኳን ወደ ዳዊት የመኪና እቁብ በደህና መጡ\n\nእባክዎ መኪና ይምረጡ፦", 
...
async def car_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice == "📢 ማስታወቂያ ላክ" and update.effective_user.id in ADMINS:
        await update.message.reply_text("ለተጠቃሚዎች የሚላክ መልእክት ወይም ፎቶ ይላኩ፦")
        return BROADCAST
    if choice == "❓ ጥያቄ ለመጠየቅ":
        await update.message.reply_text("ጥያቄዎን እዚህ ይጻፉ፤ ለአድሚኑ ይደርሳል፦")
        return QUESTION
...
async def ask_question(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ ጥያቄዎ ተልኳል! አድሚኑ መልስ ይሰጥዎታል። በ /start ይመለሱ።")
...
async def l_name_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📞 የኢትዮጵያ ስልክ ቁጥርዎን ያስገቡ (+251...)፦")
    return PHONE
...
async def screenshot_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.photo:
        ...
        await update.message.reply_text("⌛ ጥያቄዎ በሚገባ ተመዝግቧል! አድሚኑ እስኪያረጋግጥ ድረስ ትንሽ ይጠብቁ...", reply_markup=ReplyKeyboardRemove())
        return ConversationHandler.END
    return SCREENSHOT
...
async def handle_v(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if act == "v" and update.effective_user.id in ADMINS:
        conn = sqlite3.connect('lottery.db')
    ...
    elif act == "r" and update.effective_user.id in ADMINS:
        await q.edit_message_caption(q.message.caption + "\n\n❌ ተሰርዟል!")
...
async def broadcast_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for u in users:
        try:
            if update.message.text: await context.bot.send_message(u[0], update.message.text)
            elif update.message.photo: await context.bot.send_photo(u[0], update.message.photo[-1].file_id, caption=update.message.caption)
        except: pass
    await update.message.reply_text("✅ ማስታወቂያ ተልኳል!")
    return ConversationHandler.END
