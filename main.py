...
class HealthCheck(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Adane Ekub Bot is Online")
...
def init_db():
    conn = sqlite3.connect('ekub_final_v7.db')
    c = conn.cursor()
...
def main_menu_keyboard(user_id):
    ...
    keyboard = []
    # ተጠቃሚው ካልተመዘገበ ብቻ Register የሚለው ይታያል
    ...
    if user_id == ADMIN_ID:
        keyboard.append([InlineKeyboardButton("📢 ብሮድካስት (Broadcast)", callback_data="admin_broadcast")])
...
async def get_id_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("የመመዝገቢያ 200 ብር መክፈያ መንገድ?", reply_markup=InlineKeyboardMarkup(kb))
    return PAY_CHOICE
...
async def get_screenshot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    verify_kb = InlineKeyboardMarkup([[InlineKeyboardButton("Verify ✅", callback_data=f"v_reg_{user_id}")]])
...
async def ask_admin_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    await update.callback_query.message.reply_text("ጥያቄዎን ይጻፉ (ለመመለስ /start):", reply_markup=back_kb)
    return ASK_ADMIN
...
async def broadcast_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    await update.callback_query.message.reply_text("ለሁሉም የሚላክ መልዕክት ይላኩ፦", reply_markup=back_kb)
...
async def run_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("መልዕክቱ ተልኳል!", reply_markup=main_menu_keyboard(ADMIN_ID))
    return ConversationHandler.END
...
async def admin_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.close()
    await update.callback_query.answer()
    await update.callback_query.message.reply_text(f"📊 በቦቱ ውስጥ ያሉት ተጠቃሚዎች ብዛት፦ {count}", reply_markup=back_kb)
...
async def get_ekub_screenshot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    verify_kb = InlineKeyboardMarkup([[InlineKeyboardButton("Verify Weekly Pay ✅", callback_data=f"v_week_{update.effective_user.id}")]])
    await context.bot.send_photo(ADMIN_ID, photo=update.message.photo[-1].file_id, caption=f"💰 የሳምንት ክፍያ ደረሰኝ ከ: {update.effective_user.id}", reply_markup=verify_kb)
    await update.message.reply_text("ክፍያው እስኪረጋገጥ ይጠብቁ።", reply_markup=main_menu_keyboard(update.effective_user.id))
    return ConversationHandler.END
